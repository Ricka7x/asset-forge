#!/bin/bash
# Usage: ./audit-images.sh <dir> [size_threshold_kb]
# Audits a directory and reports image issues.
# Checks: oversized files, unoptimized formats, missing @2x retina variants.
# Requires: ImageMagick

TARGET="${1:?'Usage: ./audit-images.sh <dir> [size_threshold_kb]'}"
THRESHOLD_KB="${2:-200}"
...
done < <(find "$TARGET" -type f \( -name '*.png' -o -name '*.jpg' -o -name '*.jpeg' -o -name '*.gif' -o -name '*.webp' -o -name '*.avif' \))
...
done < <(find "$TARGET" -type f -name '*.gif')
[ $found -eq 0 ] && echo "  none"
echo ""

# 4. Missing @2x retina variants
echo "── Missing @2x retina variants ──────────────────────────────"
found=0
while IFS= read -r file; do
  base="${file%.*}"
  ext="${file##*.}"
  # Skip files that are themselves @2x
  [[ "$base" == *@2x ]] && continue
